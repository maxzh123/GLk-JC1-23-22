...
public class Sample3 {   //Сообщаем компилятору что Это класс и мы дали ему имя Sample3
    ...
    private static void processNumber(int num){ //определяем функцию не возвращающую результата и принимающую один целочисленный параметр
        if (num%3==0 && num!=0){ //если остаток от деления на 3 равен 0 то выполняем данный блок
            System.out.println(num+" делиться на 3");//Достаем системный поток out и вызываем его метод println (напечатать и перевести строку). В качестве аргумента передаем то, что хотим напечатать.
        } else ;
    }
}
